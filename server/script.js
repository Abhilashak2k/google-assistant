...
app.get("/location/:location/:cuisine/outlets", function(req, resp) {
    ...
    connection.getConnection(function(error, tempCont) {
        if (error) {
            ...
            //getting blogs query for use later

            tempCont.query(`SELECT DISTINCT id 
                            FROM   wp_posts 
                            ...
                            WHERE  post_type = "post" 
                                   ...
                                   AND wp_term_taxonomy.term_id IN ( 72, 73, 74, 75 ) `,
                function(error, rows, fields) {
                    ...
                    tempCont.query(`SELECT DISTINCT id, 
                                                    ...
                                                    ( 
                                                               SELECT     p.guid 
                                                               FROM       wp_postmeta2 AS pm 
                                                               INNER JOIN wp_posts     AS p 
                                                               ON         pm.meta_value=p.id 
                                                               WHERE      pm.post_id = "'.$id.'" 
                                                               AND        pm.meta_key = "_thumbnail_id") AS "image_url",
                                                    ( 
                                                               ...
                                                               AND        wp_posts.id = wpr.object_id ) AS "location" ,
                                                    ( 
                                                               SELECT     group_concat(wp_terms.NAME separator ", ")
                                                               FROM       wp_terms 
                                                               INNER JOIN wp_term_taxonomy 
                                                               ...
                                                               INNER JOIN wp_term_relationships wpr 
                                                               ON         wpr.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id
                                                               WHERE      taxonomy= "cuisine" 
                                                               AND        wp_posts.id = wpr.object_id ) AS "cuisine"
                                    FROM            wp_posts 
                                    WHERE           post_type = "post" 
                                    ...
                                    ORDER BY        id`, function(error, rows, fields) {
